...
if ($weekly_record_id > 0) {
    $query = "SELECT * FROM union_overall WHERE weekly_record_id = $weekly_record_id";
    $date_query = "SELECT title, date_from, date_to, union_overall_fields_list FROM weekly_record WHERE id = $weekly_record_id";
} else {
    $query = "SELECT * FROM union_overall";
...
if ($date_result && $date_result->num_rows > 0) {
    ...
    if ($weekly_record_id > 0) {
        ...
        $fields_list = $row['union_overall_fields_list'] ?? '';
...
// Get column headers from union_overall if not stored
...
<body>
    <div class="container mt-3">
        <div class="row mb-3">
            <div class="col-md-3">
                <a href="uniondata.php" class="btn btn-primary">Back to Union Reports</a>
            </div>
        </div>
